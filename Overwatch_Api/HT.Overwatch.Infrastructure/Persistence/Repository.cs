...
namespace HT.Overwatch.Infrastructure.Persistence
{
    public class Repository<T> : IRepository<T> where T : class
    {
        ...
        private readonly OverwatchDbContext _dbContext;

        public Repository(OverwatchDbContext dbContext)
        {
        ...
        {
            return GetQuerable().SingleOrDefault<T>(criteria);
        }

        public T GetFirst(Expression<Func<T, bool>> criteria)
        {
            return GetQuerable().FirstOrDefault<T>(criteria);
        }
